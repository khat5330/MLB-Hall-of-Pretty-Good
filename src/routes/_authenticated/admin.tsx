...
export const Route = createFileRoute("/_authenticated/admin")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Review induction posts and publish new Hall of Pretty Good inductees." },
      { property: "og:title", content: "Review queue — Hall of Pretty Good Admin" },
      { property: "og:description", content: "Review induction posts and publish new Hall of Pretty Good inductees." },
    ],
  }),
  component: AdminPage,
});
            ...
            {statusQuery.data.canClaim ? (
              <>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  No administrator exists yet. Claim it now to lock the admin area to your
                  account.
                </p>
                <button
                  type="button"
    ...
    <article className="border border-border bg-background p-4">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        ...
        {item.sourceUrl && (
          <>
            {" · "}
            <a href={item.sourceUrl} target="_blank" rel="noreferrer" className="text-primary underline">
              view post
            </a>
          </>
        )}
      ...
      <div className="mt-3 flex flex-wrap gap-4 text-sm">
        <label className="flex items-center gap-2">
          <input type="checkbox" checked={innerCircle} onChange={(e) => setInnerCircle(e.target.checked)} />
          Inner Circle
        </label>
        <label className="flex items-center gap-2">
          <input
