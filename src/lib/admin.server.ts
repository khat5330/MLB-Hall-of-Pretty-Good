...
function toQueueItem(row: any): QueueItem {
  return {
    ...
    publishedMlbId: row.published_mlb_id,
 ...
 *
 * Phase 2 always queues to 'needs_review' regardless of match confidence —
 * the auto-publish confidence rule is a phase 3 addition. `matched` here
 * uses a loose single-result fallback that's fine as a *prefill suggestion*
 * for the human reviewer in admin.tsx, but is deliberately not strict enough
 * to gate an unattended publish.
 */
...
export async function approveQueueItem(supabase: DB, input: ApproveInput) {
  ...
  const { error: qErr } = await supabase
    .from("pending_inductees")
    .update({ status: "approved", matched_mlb_id: input.mlbId, published_mlb_id: input.mlbId })
    .eq("id", input.id);
